...
class BankingAssistant:
    ...
    def router_node(self, state: AgentState):
        ...
        if any(word in query for word in banking_keywords): return "retrieve"
        if any(word in query for word in ["time", "calculate", "date"]): return "tool"
        return "skip"

    def retrieval_node(self, state: AgentState):
        results = self.vectorstore.similarity_search(state['messages'][-1].content, k=3)
    ...
    def skip_retrieval_node(self, state: AgentState):
        return {"context": "General conversation logic. No KB context used."}

    def tool_node(self, state: AgentState):
        try:
            now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            return {"context": f"System Status: Online. Current Date/Time: {now}."}
        except Exception as e:
            return {"context": f"Tool Error: {str(e)}"}

    def answer_node(self, state: AgentState):
    # 1. EXTRACT CONTEXT FROM HISTORY
    # We scan the history to see if 'rural', 'urban', or a name was mentioned previously
        history_text = " ".join([m.content for m in state['messages'][:-1]])
        current_query = state['messages'][-1].content
    
    # 2. ENFORCED SYSTEM PROMPT
    # We explicitly tell the AI to look at the history we just extracted
    ...
    def eval_node(self, state: AgentState):
        ...
        try:
            score = float(self.llm.invoke(eval_q).content.strip())
        except: score = 0.8
        return {"faithfulness_score": score, "retry_count": state['retry_count'] + 1}
    ...
    def compile_workflow(self):
        ...
        builder.add_conditional_edges("evaluate", lambda x: "retry" if x["faithfulness_score"] < 0.7 and x["retry_count"] < 3 else "end", {"retry": "answer", "end": "save"})
